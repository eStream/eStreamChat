/* This file is part of eStreamChat.
 * 
 * eStreamChat is free software: you can redistribute it and/or modify
 * it under the terms of the GNU Affero General Public License as published by
 * the Free Software Foundation, either version 3 of the License, or
 * (at your option) any later version. 
 * 
 * eStreamChat is distributed in the hope that it will be useful,
 * but WITHOUT ANY WARRANTY; without even the implied warranty of
 * MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the
 * GNU Affero General Public License for more details.
 * 
 * You should have received a copy of the GNU Affero General Public License
 * along with eStreamChat. If not, see <http://www.gnu.org/licenses/>.
 */
using eStreamChat.Interfaces;
using eStreamChat.Properties;

namespace eStreamChat.Classes
{
    public class ChatSettings : IChatSettings
    {
        #region IChatSettings Members

        public bool EnableFileTransfer
        {
            get { return Settings.Default.EnableFileTransfer; }
        }

        public string SendFileAllowedExtensions
        {
            get { return Settings.Default.SendFileAllowedExtensions; }
        }

        public bool EnableVideoChat
        {
            get { return Settings.Default.EnableVideoChat; }
        }

        public string FlashMediaServer
        {
            get { return Settings.Default.FlashMediaServer ; }
        }

        public string FlashServerType
        {
            get { return Settings.Default.FlashServerType ; }
        }

        #endregion
    }
}